...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/07/15 14:08:00 by dadavyde          #+#    #+#             */
/*   Updated: 2018/07/15 14:08:00 by dadavyde         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_coord_sys		init_basis_after_rot(t_rt *rt_data)
{
	...
	t_vector		axis_x_in_cam_sys;
	t_vector		axis_y_in_cam_sys;

	new_basis = matrix_mult_matrix(rt_data->camera.initial_basis,
						init_rot_matrix_for_axis(rt_data->camera.angle_rot.z,
															VEC(0, 0, 1)));
	axis_y_in_cam_sys = matrix_mult_vect(count_inverse_matrix(new_basis),
														new_basis.b_y);
	new_basis = matrix_mult_matrix(new_basis,
						init_rot_matrix_for_axis(rt_data->camera.angle_rot.y,
															axis_y_in_cam_sys));
	axis_x_in_cam_sys = matrix_mult_vect(count_inverse_matrix(new_basis),
															new_basis.b_x);
	new_basis = matrix_mult_matrix(new_basis,
						(init_rot_matrix_for_axis(rt_data->camera.angle_rot.x,
													axis_x_in_cam_sys)));
	return (new_basis);
}
